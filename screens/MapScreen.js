...
export default function MapScreen() {
  ...
  useEffect(() => {
    fetch(`http://192.168.1.106:3000/places/${user.nickname}`)
      .then(response => response.json())
      .then(marker => {
        if(marker.result){
          
          marker.places.map((mark) => {
            console.log(mark);
            dispatch(addPlace({ name: mark.name, latitude: mark.latitude, longitude: mark.longitude }))
          })
        }})
  }, [])

  const handleLongPress = (e) => {
    //POST NEW PLACE A METTRE ICI, MAIS SANS LE NOM?
    //rajouter le nom dans TEMPCOORDINATES ou créer un USESTATE
  ...
  const handleNewPlace = () => {
    ...
    fetch('http://192.168.1.106:3000/places', {
      method: 'POST',
      headers: { 'Content-Type' : 'application/json'},
      body: JSON.stringify(newMarke)
    })
      .then(response => response.json())
